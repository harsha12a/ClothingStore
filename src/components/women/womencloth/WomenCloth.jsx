...
function WomenCloth({x}) {
  let {setErr} = useContext(loginContext)
    ...
    const addToCart= async ()=>{
      ...
      try{
        let user = sessionStorage.getItem('user')
        user = JSON.parse(user)
        if(user.cart.filter(x => x.id === products.id && x.type === products.type).length > 0){
        ...
        let res = await axios.put(`http://localhost:5000/user/addcart/${user.username}`,newUser)
  ...
  return (
    <div className='p-3 d-flex flex-column justify-content-center m-3 align-items-center border border-dark'>
        ...
        <button className='btnhov' onClick={()=>addToCart()}>Add to Cart</button>
        <ToastContainer />
    </div>
  )
}
